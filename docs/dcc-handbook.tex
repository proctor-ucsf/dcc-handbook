...
Depending on the project, each step involves a different amount of work. Step 1 is by far the most time consuming, and often the most error-prone. We devote an entire chapter to it below (\hyperref[datawrangling]{Data wrangling})
...
A key tool to help ensure your R code runs reliably on different machines and over time is \texttt{renv} (\hyperref[renv]{\texttt{renv}}).
...
\emph{Contributors: Christine Tedijanto and Ben Arnold}
...
For more on reproducible environments, see the Turing Way section \href{https://the-turing-way.netlify.app/reproducible-research/renv.html}{here}.

\section{\texorpdfstring{\texttt{renv}}{renv}}\label{renv}

A package management system is used to track downloaded packages and their versions. One common package management system in R is the \texttt{renv} package. The package has nice documentation that can be found \href{https://rstudio.github.io/renv/articles/renv.html}{here}. When using \texttt{renv}, the state of your project library will be saved to a file in your project called \texttt{renv.lock}.
...
\bottomrule\noalign{}
\endlastfoot
\texttt{renv::init()} & to initialize a project-local environment with a private R library \\
\texttt{renv::snapshot()} & to save the current state of the project library to the \texttt{renv.lock} file \\
\texttt{renv::restore()} & to revert to the state of the project library previously saved in the \texttt{renv.lock} file \\
\end{longtable}

\section{Docker}\label{docker}

\href{https://docs.docker.com/}{Docker} is a platform that allows for containerizing environments. The environments can then be shared and launched on a different machine. In our case, we are will often be launching an online browser version of RStudio with a pre-specified R version and packages.

To get started with Docker, download and install Docker on your local machine. Docker is run using the command line, and the accompanying desktop application provides information on which containers and images are currently running. Docker \textbf{images} are the outline for an environment that can be either downloaded or built locally. Images are built using instructions in a text file called a \texttt{Dockerfile}. Although it may take additional time to build an image locally, sharing the \texttt{Dockerfile} allows users to see all the steps that were taken to build the image. A \textbf{container} is an instance of a particular image.

The \href{https://www.rocker-project.org/}{Rocker Project} includes many Docker images that are good starting points for building a reproducible environment with R. More information can be found in this \href{https://journal.r-project.org/archive/2017/RJ-2017-065/RJ-2017-065.pdf}{article} by the creators of Rocker.

Data can also be shared from your local machine to the Docker container as \href{https://docs.docker.com/storage/volumes/}{volumes}.

Our experience so far is that Docker can be really great, but it can also be a hassle! The most important step in a reproducible environment is \texttt{renv} (above). Add Docker on top of \texttt{renv} for extra syle points, but never omit \texttt{renv}!

\section{\texorpdfstring{Putting \texttt{renv} and Docker together}{Putting renv and Docker together}}\label{putting-renv-and-docker-together}

Docker and \texttt{renv} can be combined to define reproducible environments including the operating system, R version, R packages and respective versions, and other dependencies. The \texttt{renv} team has written documentation on how to combine the systems \href{https://rstudio.github.io/renv/articles/docker.html}{here}.

For an example using Docker and \texttt{renv} together to recreate a computational environment, see the repository \href{https://github.com/proctor-ucsf/trachoma-sero-transmission}{here}.

Other approaches to building reproducible environments include \href{https://mybinder.org/}{Binder}.
